    ...
    <div className="flex flex-wrap items-center gap-2">
      {caps.canManage && status === 'draft' ? (
        <button
          ...
          className={btn}
          disabled={pending}
          onClick={() => act(() => submitVersionForApproval(versionId))}
        >
          Submit for approval
        </button>
      ) : null}
      {caps.canApprove && status === 'pending_approval' ? (
        <button
          ...
          className={btn}
          disabled={pending}
          onClick={() => act(() => approveVersion(versionId))}
        >
          Approve
        </button>
      ) : null}
      {caps.canManage && (status === 'pending_approval' || status === 'draft') ? (
        <button
          ...
          disabled={pending}
          onClick={() => act(() => activateVersion(versionId))}
        >
          Activate
        </button>
      ) : null}
      {caps.canManage && status === 'active' ? (
        <button
          ...
          className={btn}
          disabled={pending}
          onClick={() => act(() => retireVersion(versionId))}
        >
          Retire
        </button>
      ) : null}
    ...
    <span className="inline-flex items-center gap-1">
      ...
      <button
        ...
        onClick={() =>
          start(async () => {
            setError(null);
            const res = await createDraftVersion({ modelId, sourceVersionId, version: label });
            if (!res.ok) setError(res.error ?? 'Failed.');
            else setOpen(false);
          })
        }
